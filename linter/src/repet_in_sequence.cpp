...
void checkRepetitionInSequence(const FileContent* fC) {
    ...
    for (NodeId seqDeclId : seqDecls) {
        ...
        for (NodeId seqExprId : seqExprs) {
            ...
            auto gotoNodes = fC->sl_collect_all(seqExprId, VObjectType::paGoto_repetition);
            auto nonConsecNodes = fC->sl_collect_all(seqExprId, VObjectType::paNon_consecutive_repetition);
            ...
            if (hasGoto && hasNonConsec) {
                ...
                std::cerr << "Error REPETITION_IN_SEQUENCE: sequence '"
                          << seqName
                          << "' uses both goto '[->]' and non-consecutive '[=]' repetitions at "
